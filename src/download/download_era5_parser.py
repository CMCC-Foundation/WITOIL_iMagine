...
def get_era5(xmin, xmax, ymin, ymax, start_date, end_date, output_path, output_name):
    ...
    for i in range(0, days):
        ...
        outputname = (
            output_path
            + f"temp_{str(date.year)+str(date.month).zfill(2)+str(date.day).zfill(2)}.nc"
        )
...
def process_era5(output_path, output_name):
    ...
    met = xr.open_mfdataset("input_data/atmosphere/ERA5/temp*.nc")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Download wind fields for a specific area and time window."
    )
